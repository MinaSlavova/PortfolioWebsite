...
function App() {
  const imagePath = (folder, name) => `/images/${folder}/${name}`;
  const videoPath = (folder, name) => `/videos/${folder}/${name}`;
  ...
  const mockProjects1 = [
    {
      ...
      media: [
        "HackTUES-banner.png",
        "TUES-Fest-banner.png",
        "HackTUES-banner-temp.png",
        "HackTUES-Mayura-expression-sheet.png",
        "HackTUES-sticker-art.png",
        "HackTUES-sticker-pic.jpg",
        "TUES-team-design.png",
        "TUES-team-PR.png",
      ...
      description:
        "HackTUES is a three day hackaton in Bulgaria, organized for students by students. It is an annual event which changes organizers every year. In 2022 I got to be a part of the organizing team as a designer. As part of a two-person design team, I contributed to the event’s visual identity, designing assets such as the logo, social media posts, and merchandise. I was also personally responsible for illustrating the first ever HackTUES comic, which later became part of the yearly tradition.",
...

    {
      ...
      media: [
        ...[
          "SpliceBorn-trailer.mov",
        ].map((name) => ({
          type: "video",
          src: videoPath("SpliceBorn", name),
        })),
        ...[
          "SpliceBorn-menu.png",
          "SpliceBorn-character-attack.gif",
        ].map((name) => ({
          type: "image",
          src: imagePath("SpliceBorn", name),
        })),
      ],
      description:
...

    {
      ...
      media: [
        ...[
          "ReFocus-app-screens.png",
          "ReFocus-brand-guide.png",
        ].map((name) => ({
          type: "image",
          src: imagePath("ReFocus", name),
        })),
      ],
      description:
